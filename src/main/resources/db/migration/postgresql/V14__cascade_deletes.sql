-- Cascade-delete user/group children so account, group, and task deletion leave no orphan rows (§4.20),
-- and fix the task_photos bytea-blob leak (§4.19). The V1 baseline declared almost no foreign keys, so
-- deleting a user/group/task stranded rows in many tables. Add ON DELETE CASCADE (and SET NULL where the
-- row must survive). Existing orphans are purged first so each ADD CONSTRAINT validates.
--
-- Deliberately NO FK on tasks.owner_id or family_groups.owner_id: account deletion must transfer owned
-- groups and delete only PERSONAL tasks (group tasks the user created survive with the group). That stays
-- orchestrated in AccountDeletionService.
...
-- owner_id is intentionally left without a FK (see header).
